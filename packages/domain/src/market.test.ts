...
describe('computeDiscoveryScore', () => {
  it('ranks a high-volume, high-liquidity market above a quiet one', () => {
    const hot = computeDiscoveryScore({ volume24hUsd: 5_000_000, liquidityUsd: 2_000_000, priceChange24hPct: 12 });
    const quiet = computeDiscoveryScore({ volume24hUsd: 20_000, liquidityUsd: 50_000, priceChange24hPct: 1 });
  ...
  it('does not let an absurd percentage swing on a tiny denominator dominate the score', () => {
    ...
    const absurd = computeDiscoveryScore({ volume24hUsd: 100_000, liquidityUsd: 100_000, priceChange24hPct: 5000 });
  ...
  it('returns null instead of a misleading zero when volume/change data is missing', () => {
    expect(computeDiscoveryScore({ volume24hUsd: null, liquidityUsd: 1_000_000, priceChange24hPct: 5 })).toBeNull();
    expect(computeDiscoveryScore({ volume24hUsd: 1000, liquidityUsd: 1_000_000, priceChange24hPct: null })).toBeNull();
  });
});
