      ...
      <main className="mx-auto max-w-3xl px-6 py-10">
        ...
        {profile.stats.totalSwaps > 0 && (
          <Surface className="mt-6 p-5">
            <h2 className="mb-4 font-display text-sm font-semibold text-ink-900">Trading behavior</h2>
            <p className="mb-4 font-body text-xs text-ink-400">
              Derived only from this wallet&apos;s own confirmed, indexed trades — not investment advice. See
              docs/TRADER_INTELLIGENCE.md.
            </p>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              <Stat label="Unique tokens" value={profile.stats.uniqueTokensTraded.toString()} />
              <Stat label="Avg trade size" value={formatCompactUsd(profile.stats.avgTradeSizeUsd)} />
              <Stat label="Largest trade" value={formatCompactUsd(profile.stats.largestTradeUsd)} />
              <Stat
                label="Buy ratio"
                value={profile.stats.buyRatio !== null ? `${(profile.stats.buyRatio * 100).toFixed(0)}%` : '—'}
              ...
              <Stat
                label="Concentration"
                value={profile.stats.concentrationIndex !== null ? `${(profile.stats.concentrationIndex * 100).toFixed(0)}%` : '—'}
                title="How concentrated this wallet's volume is in a single token — higher means less diversified across tokens."
              />
              <Stat
                label="Trades / day"
                value={profile.stats.activityFrequencyPerDay !== null ? profile.stats.activityFrequencyPerDay.toFixed(2) : '—'}
              />
            </div>
          </Surface>
        )}
