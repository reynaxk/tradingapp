 ...
 * lib/session-client.ts: follow/like mutations and the live activity feed. This is one of
 * the places apps/web talks to the API directly from the browser rather than through a
 * Server Component — SSE and user-triggered mutations both structurally require a real
...
export async function checkFollowStatus(address: string): Promise<boolean> {
  ...
  try {
    const res = await authedFetch(`/social/traders/${encodeURIComponent(address)}`);
